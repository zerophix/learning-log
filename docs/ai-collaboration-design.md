...
AI 执行深度分析（按 Skill 指令的要求）:
  ...
  ├─ 3. Generate: 生成 ≥1200 字的协议对齐深度分析
  │    ├─ 一句话结论
  │    ├─ 系统全景 (Mermaid + ASCII)
  │    ├─ 为什么 (Why — 第一性原理)
  │    ├─ 架构与流程 (ASCII 树状/流程 + Mermaid)
  │    ├─ 关键决策 (对比表)
  │    ├─ STAR 复盘 + 迁移
...
构造 payload → learnlog CLI:
  {
    "topic": "...",
    "insight": "...(≥1200 字，含 Mermaid/ASCII 图)",
    "diagram": "graph TD\n  ...",
    "code_snippet": "...",
    "star_situation": "...",
    "star_task": "...",
...
### 协议对齐深度分析结构（Generate 阶段强制输出）

每条学习记录的 `insight` 字段必须遵循个人 AI 协作协议：

| # | 段落 | 内容要求 |
|---|------|---------|
| 1 | **一句话结论** | 一句话总结，结论先行，30 秒抓要点 |
| 2 | **系统全景** | Mermaid 图 + ASCII 结构图，视觉优先 |
| 3 | **为什么 (Why)** | 第一性原理：回归底层，拆解本质因果（可跳过） |
| 4 | **架构与流程** | ASCII 树状/流程 + Mermaid，展示结构与步骤 |
| 5 | **关键决策** | 替代方案对比表：优缺点 + 选择理由 |
| 6 | **STAR + 迁移** | S/T/A/R 四维复盘 + 跨领域迁移（如何用于其他问题） |

**字数**：≥ 1200 字。图多文少，降低连续长段文字。每章节独立，同一件事不在两个章节复用。
...
T+8min  用户: "/记录"
        AI: 
          ...
          ├─ 检查标签 "cn...discipline.cs.backend.python" → 已存在
           ├─ 生成 1200+ 字协议对齐分析:
  │   ├─ 一句话结论：确立以唯一索引为基石的幂等性设计
  │   ├─ 系统全景：Mermaid 架构图 + ASCII 层级
  │   ├─ Why：数据一致性原则 + 反熵设计
  │   ├─ 架构与流程：ASCII 时序图 (客户端→应用层→数据库层)
  │   ├─ 关键决策：对比唯一索引 vs 分布式锁
  │   └─ STAR + 迁移：亿级数据下 Hash 索引的瓶颈？
          ├─ 构造 22 字段 payload
...
可以看到：
- **insight** 是完整的协议对齐分析长文（通常 1200+ 字，含 Mermaid/ASCII 图）
...
| **强制深度** | 协议对齐格式（结论→图→Why→架构→决策→STAR），防止肤浅记录 |
