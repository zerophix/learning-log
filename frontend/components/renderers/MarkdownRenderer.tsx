...
function splitByMermaid(content: string): { type: 'markdown' | 'mermaid'; content: string }[] {
...
export default function MarkdownRenderer({ content }: { content: string }) {
  const parts = useMemo(() => splitByMermaid(content), [content]);
      ...
      <ErrorBoundary fallback={<div style={{ padding: '20px', color: 'var(--text-muted)', fontSize: '13px' }}>内容渲染异常，请检查 Markdown 语法</div>}>
        {parts.map((part, i) => {
          if (part.type === 'mermaid') {
            return <MermaidDiagram key={`m-${i}`} chart={part.content} />;
          }
          return (
            <ReactMarkdown
              key={`md-${i}`}
              ...
              components={{
                h1: ({ children }) => (
                  <h1 style={{
                    ...
                    margin: '32px 0 16px 0', paddingBottom: '12px',
                    borderBottom: '2px solid var(--border-color)', letterSpacing: '-0.02em'
                  }}>{children}</h1>
                ),
                h2: ({ children }) => (
                  <h2 style={{
                    fontSize: '18px', fontWeight: 600, color: 'var(--text-primary)',
                    margin: '28px 0 14px 0', paddingBottom: '8px',
                    borderBottom: '1px solid var(--border-color)', letterSpacing: '-0.01em'
                  }}>{children}</h2>
                ),
                h3: ({ children }) => (
                  <h3 style={{
                    fontSize: '15px', fontWeight: 600, color: '#E2E8F0',
                    margin: '24px 0 12px 0', letterSpacing: '-0.01em'
                  }}>{children}</h3>
                ),
                h4: ({ children }) => (
                  <h4 style={{
                ...
                ul: ({ children }) => (
                  <ul style={{
                    fontSize: '14px', lineHeight: '1.75', color: 'var(--text-secondary)',
                    margin: '0 0 16px 0', paddingLeft: '24px'
                  }}>{children}</ul>
                ),
                ol: ({ children }) => (
                  <ol style={{
                ...
                blockquote: ({ children }) => (
                  <blockquote style={{
                    borderLeft: '4px solid var(--border-color)', paddingLeft: '16px',
                    margin: '20px 0', color: 'var(--text-secondary)', fontSize: '14px',
                    lineHeight: '1.75', background: 'rgba(51, 65, 85, 0.3)',
                ...
                a: ({ children, href }) => (
                  <a href={href} style={{
                    color: 'var(--accent-sky)', textDecoration: 'none',
                    borderBottom: '1px solid rgba(56, 189, 248, 0.3)', transition: 'border-color 0.2s'
