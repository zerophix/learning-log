...
# 安装 MCP SSE 服务（供 Cline / OpenCode 等 AI 代理接入）
...
- AI 模型:       Ollama (qwen3-coder:30b，可在 backend/.env 中切换)
...
┌───────────────────────────────────────────────────────┐
│  AI 代理层                                            │
│  Claude Code · Cline · OpenCode · 任意 HTTP 客户端     │
└──────────────────────┬────────────────────────────────┘
                       │ MCP / HTTP / CLI
┌──────────────────────▼────────────────────────────────┐
│  服务层                                               │
│  MCP SSE Server (:8010)  ← 所有 AI 代理均可接入        │
...
┌──────────▼──────────┐  ┌─────────────▼──────────────┐
│  AI 层              │  │  数据层                     │
│  qwen3-coder:30b    │  │  SQLite · 四层标签体系       │
│  (本地 Ollama)       │  │  (discipline→subject→topic→entry)│
└─────────────────────┘  └────────────────────────────┘
```
...
| 目录 | 说明 |
|------|------|
| `backend/` | FastAPI + SQLite + MCP Server + .env(AI配置) |
| `frontend/` | Next.js 14 时间线 / ECharts 图谱 / Feed 筛选 / SVG Logo |
| `deploy/` | launchd plist + MCP/后端/前端 启动脚本 |
| `scripts/tools/` | 可复用工具（mcp_manager, auto_record...） |
| `scripts/seeds/` | 种子数据（四层标签体系） |
| `install/` | Shell 集成（learnlog CLI 源文件） |
| `.claude/skills/` | Claude Code 中文短命令 Skill |
| `~/.config/opencode/skills/` | OpenCode 全局 Skill |
| `docs/` | 设计文档（分类存放：architecture/ backend/ frontend/ graph/ guide/ assets/） |
